...
pub fn createAvatar(
    ...
    // Link equipped lightcone from store
    for (equipment_store[0..equipment_count]) |eq| {
        if (eq.dress_avatar_id == avatarConf.id and eq.unique_id != 0) {
            avatar.equipment_unique_id = eq.unique_id;
            break;
        }
    }
    return avatar;
...
pub fn createAllAvatar(
    ...
    // Link equipped lightcone from store
    for (equipment_store[0..equipment_count]) |eq| {
        if (eq.dress_avatar_id == Avatar_id and eq.unique_id != 0) {
            avatar.equipment_unique_id = eq.unique_id;
            break;
        }
    }
...
pub fn createRelic(
    ...
    var r = protocol.Relic{
        ...
        .EBEOECJMBKN = ArrayList(protocol.RelicAffix).init(allocator),
    };
