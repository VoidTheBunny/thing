...
pub const SceneManager = struct {
    ...
    fn addAvatarEntities(
        ...
        for (avatar_ids, 0..) |id, i| {
            if (id == 0) continue;
            const base_id = getBaseAvatarId(id);
            const offset_pos = protocol.Vector{
                .x = tele_pos.x + @as(i32, @intCast(i)) * 100,
                .y = tele_pos.y,
                .z = tele_pos.z,
            };
            try scene_group.entity_list.append(.{
                ...
                .motion = .{ .pos = offset_pos, .rot = tele_rot },
            });
        }
    }
    fn addPropEntities(
    ...
    pub fn createScene(
        ...
        const leader_id = getBaseAvatarId(BattleManager.selectedAvatarID[0]);
        scene_info.leader_entity_id = leader_id + 100000;
        ...
        for (res_config.scene_config.items) |sceneConf| {
            for (sceneConf.teleports.items) |teleConf| {
                ...
                scene_group.state = 1;
                try addAvatarEntities(&scene_group, &BattleManager.selectedAvatarID, toVector(teleConf.pos), toVector(teleConf.rot), 0);
