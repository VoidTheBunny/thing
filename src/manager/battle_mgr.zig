...
fn commonBattleSetup(
    ...
    for (selected_avatar_ids) |selected_id| {
        ...
        if (!found) {
            const default_conf = Config.Avatar{
                .id = selected_id,
                .hp = 100,
                .sp = 50,
                .level = 80,
                ...
                .rank = 6,
                .use_technique = true,
                .lightcone = .{ .id = 23000, .rank = 1, .level = 80, .promotion = 6 },
                .relics = std.ArrayList(Config.Relic).init(allocator),
            };
            const avatar = try createBattleAvatar(allocator, default_conf);
            try addTechniqueBuffs(allocator, battle, avatar, default_conf, avatarIndex);
            try battle.battle_avatar_list.append(avatar);
            avatarIndex += 1;
        }
    }
