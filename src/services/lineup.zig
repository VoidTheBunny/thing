...
pub fn onReplaceLineup(session: *Session, packet: *const Packet, allocator: Allocator) !void {
    ...
    // Update the battle manager's selected avatars to match the new lineup
    for (0..BattleManager.selectedAvatarID.len) |i| {
        BattleManager.selectedAvatarID[i] = if (i < ids.items.len) ids.items[i] else 0;
    }
