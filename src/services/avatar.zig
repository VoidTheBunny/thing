...
pub fn onDressAvatar(session: *Session, packet: *const Packet, allocator: Allocator) !void {
...
pub fn onTakeOffEquipment(session: *Session, packet: *const Packet, allocator: Allocator) !void {
    ...
    // Find equipment currently on this avatar and unequip it
    for (AvatarManager.equipment_store[0..AvatarManager.equipment_count]) |*eq| {
        if (eq.dress_avatar_id == req.avatar_id) {
            eq.dress_avatar_id = 0;
...
pub fn onDressRelicAvatar(session: *Session, packet: *const Packet, allocator: Allocator) !void {
    const req = try packet.getProto(protocol.DressRelicAvatarCsReq, allocator);
    defer req.deinit();

    try session.send(CmdID.CmdDressRelicAvatarScRsp, protocol.DressRelicAvatarScRsp{
        .retcode = 0,
    });
}

pub fn onTakeOffRelic(session: *Session, packet: *const Packet, allocator: Allocator) !void {
    const req = try packet.getProto(protocol.TakeOffRelicCsReq, allocator);
    defer req.deinit();

    try session.send(CmdID.CmdTakeOffRelicScRsp, protocol.TakeOffRelicScRsp{
        .retcode = 0,
    });
}
